...
每个镜头聚焦**单一动作**，描述要详尽具体。每个镜头时长必须为 1-10 秒，并根据剧情节奏分配长短，不能机械地让所有镜头使用同一时长。
...
9. **画面结果**：动作的即时后果 + 视觉细节
10. **氛围**：光线 + 色调 + 声音 + 整体氛围
11. **时长**：每个镜头 1-10 秒；对白/动作密集镜头可 8-10 秒，反应/转场/空镜可 2-5 秒
12. **静态画面提示词**：`image_prompt`，用于首帧/尾帧/镜头图片生成
13. **视频提示词**：`video_prompt`，按实际 `duration` 分段的视频生成描述（必填）
...
每个镜头必须包含 `video_prompt` 字段，用于驱动 AI 视频生成；时间码必须匹配该镜头 `duration`，最后一段可以不足 3 秒，不得出现超过 `duration` 的时间码。
