...
**duration时长估算规则（秒）**：
- **每个镜头时长不得超过5秒，必须在1-5秒范围内**。若某段剧情按内容估算会超过5秒，必须拆成两个或多个镜头，每个镜头仍不超过5秒。
- **拆分原则**：宁可多拆镜头，也不得出现 6 秒及以上的单镜；长对话、长动作要按时间或动作节点拆成多镜。

**估算步骤**：
1. **单镜时长上限为5秒**。先按内容估算该段需要的时长，若超过5秒则必须拆镜：
   - 例如一段“推门+走进去+说一句长台词”若估算为8秒 → 拆成 2 个镜头（如：镜头1 推门+走进 约4秒，镜头2 说台词 约4秒）。
2. **基础时长**（单镜内，不超过5秒）：
   - 纯对话（无明显动作）：2-4秒，台词多则拆成多镜
   - 纯动作（无对话）：2-5秒，动作复杂则拆成多镜
   - 对话+动作混合：3-5秒，超过5秒则拆镜

3. **对话与动作**：单镜内对话或动作若会导致时长超过5秒，一律拆成多个镜头，每个镜头 duration 填 1-5 秒。

4. **最终时长** = 每个镜头的 duration 必须 ≤5 秒。所有分镜时长之和 = 剧集总时长。

**示例**（均为≤5秒）：
- "陈峥转身离开"（简单动作）：1 个镜头，duration=3 或 4 秒
- "李芳：\"你要去哪里？\""（短对话）：1 个镜头，duration=3 秒
- "陈峥推开房门，李芳：\"终于找到你了，这些年你去哪了？\""（动作+中等对话，若估超过5秒）→ 拆成 2 镜：镜头1 推门 约3秒，镜头2 李芳说台词 约4秒
- "两人在雨中激烈搏斗，陈峥：\"住手！\""（复杂动作+对话）→ 拆成 2 或 3 镜，每镜 3-5 秒

**重要**：每个镜头的 duration 不得超过5秒；内容超过5秒的必须拆成多镜。所有分镜时长之和将作为剧集总时长。
...
- **每个镜头的 duration 必须≤5秒**：若某段内容超过5秒，必须拆成两个或多个镜头，每个镜头仍≤5秒
- 每个镜头只描述一个主要动作
- 区分主镜（is_primary: true）和链接镜（is_primary: false）
- 确保情绪节奏有变化
- **duration字段至关重要**：每个镜头填1-5秒，所有分镜时长之和将作为剧集总时长
- 严格按照JSON格式输出
